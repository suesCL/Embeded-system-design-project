...
int main(void){  unsigned long i,last,now;
  ...
  EnableInterrupts();           // enable interrupts for the grader
	...
	//LED toggle only either of the switch closes 
	while(1){  
		switch_1 = GPIO_PORTF_DATA_R&0x10;
		switch_2 = GPIO_PORTF_DATA_R&0x01;
		
		//led toggle if either of switch is pressed
		if (switch_1 == 0x00 | switch_2 == 0x00){
		  GPIO_PORTF_DATA_R ^= 0x02; //toggle LED at PF
			if(i<50){
      now = NVIC_ST_CURRENT_R;
      Time[i] = (last-now)&0x00FFFFFF;  // 24-bit time difference
      Data[i] = GPIO_PORTF_DATA_R&0x13; // collect bit 0,1,4
      last = now;
      i++;
      }
		}else{
